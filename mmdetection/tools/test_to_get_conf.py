...
def coco_eval_substitude(outputs, dataset, show=False):
    ...
    #best = np.where(f1s.sum(axis=1)==f1s.sum(axis=1).max())[0][-1]
    bests = np.where(f1s[:,head_col].max()==f1s[:,head_col])[0]
    bests = np.where(f1s.sum(axis=1)==f1s[np.where(f1s[:,head_col].max()==f1s[:,head_col])].sum(axis=1).max())[0]
    print("***Please set score thr to %s~%s, to get class(es) performance of %s"%(axis_conf[bests[0]], axis_conf[bests[-1]], f1s[bests[0]]))
    #model selection:
    ...
    for label in dataset.cat_ids:
        ...
        for i in range(len(dataset)):
            ...
            if false_positives[-1]>0 and label==head_col+1:
                ...
                if not os.path.exists("false_positives/%s"%fp_name):
                    shutil.copy(dataset.img_prefix+fp_name, "false_positives/")
                    shutil.copy(dataset.img_prefix+fp_name.replace('.png','.xml'), "false_positives/")
        ...
        if recall_single+precision_single!=0:
            ...
            f1_score = 0
        f1_scores[dataset.cat2label[label]] = np.round(f1_score,8)
        precisions[dataset.cat2label[label]] = np.round(precision_single,8)
        recalls[dataset.cat2label[label]] = np.round(recall_single,8)
