...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def aug_test(self, imgs, img_metas, proposals=None, rescale=False):
        ...
        det_bboxes, det_labels = box_results_with_nms_and_limit(merged_bboxes, merged_scores, rcnn_test_cfg.score_thr,
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                segm_result = self.mask_head[-1].get_seg_masks(
                    merged_masks, _det_bboxes, det_labels, rcnn_test_cfg,
                    ori_shape, scale_factor=img_metas[0][0]['scale_factor'], rescale=rescale)
            ms_segm_result['ensemble'] = segm_result
        if self.with_mask:
            results = (ms_bbox_result['ensemble'],
                       ms_segm_result['ensemble'])
