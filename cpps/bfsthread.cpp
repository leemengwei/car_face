...
BFSThread::BFSThread()
{
    ...
    GlobalValue::BFSimageData00.resize(640*512);
    GlobalValue::BFSimageData00.fill(0x00,640*512);
    GlobalValue::BFSimageData01.resize(640*512);
    ...
    if (IsAvailable(ptrStringSerial) && IsReadable(ptrStringSerial))
    {
        ...
        cout << "Device serial number1 retrieved as " << deviceSerialNumber << "..." << endl;
    }
    cout << endl;

    gcstring deviceSerialNumber1("18174560");
    if (deviceSerialNumber==deviceSerialNumber1)
    {
        qDebug()<<"change";
        GlobalValue::BFSFlag_change=true;
    }
...
void BFSThread::run()
{
    ...
    //ConfigureAcquisitionFrameRate(nodeMap,1);
    ...
    while(1)
    {
        try
        {
           ...
           ImagePtr pResultImage = pCam->GetNextImage();
            ...
            {
                sendyanshi();
                cout<<"BFSimage --------------------------------------------" << GlobalValue::timebfs.elapsed() / 1000.0 << "s"<<endl;
...
//                convertedImage->Save(filename.str().c_str());
                ...
                switch(GlobalValue::BFS1_imagenum0)
                {
                    case 0:
                    {GlobalValue::BFS1_imagenum0++;
                    qDebug()<<"BFSimage --------------------------------------------" << GlobalValue::timebfs.elapsed() / 1000.0 << "s";
                    if(GlobalValue::timebfs.elapsed()>60)
                    {
                        //sendyanshi();
                    }
                    memcpy(GlobalValue::BFSimageData00.data(),(uchar *)convertedImage->GetData(),640*512);
                    sendBFSimage00();
                    break;
                    }
                    case 1:
                    ...
                    GlobalValue::BFS1_imagenum0++;
                    //qDebug()<<"BFSimage --------------------------------------------" << GlobalValue::timebfs.elapsed() / 1000.0 << "s";
                    memcpy(GlobalValue::BFSimageData01.data(),(uchar *)convertedImage->GetData(),640*512);
                    sendBFSimage01();
                    break;
                    }
                    default:
...
int BFSThread::ResetTrigger(INodeMap & nodeMap)
{
    int result = 0;
...
int BFSThread::ConfigureTrigger(INodeMap & nodeMap)
{
    int result = 0;
    ...
    try
    {
        ...
        else if (chosenTrigger == HARDWARE)                                            //全局变量chosenTrigger选择为硬件触发时*******************
        {
            ...
            cout << "Trigger source set to hardware line0..." << endl;

//bzq add
            CEnumerationPtr ptrTriggerActivation = nodeMap.GetNode("TriggerActivation");  //  1, 选择设置节点********************

            if (!IsAvailable(ptrTriggerActivation) || !IsWritable(ptrTriggerActivation))
            {
                cout << "Unable to set TriggerActivation (node retrieval). Aborting..." << endl;
                return -1;
            }

            ptrTriggerActivation->SetIntValue(ptrTriggerActivation->GetEntryByName("FallingEdge")->GetValue());//  3, 设置生效********************

            cout << "Trigger Activation FallingEdge..." << endl;
...
int BFSThread::ConfigureExposure(INodeMap& nodeMap,double exposureTimeToSet)
{
    int result = 0;
...
int BFSThread::ConfigureExposurenodeMap(double exposureTimeToSet)
{
...
int BFSThread::ConfigureGain(INodeMap& nodeMap,double GainToSet)
{
    int result = 0;
...
int BFSThread::ConfigureGainnodeMap(double GainToSet)
{
...
int BFSThread::ConfigureUserSet(CameraPtr pCam)
{
    int result = 0;
    int err = 0;
    try
...
int BFSThread::ConfigureAcquisitionFrameRate(INodeMap& nodeMap,double AcquisitionFrameRateToSet)
{
    int result = 0;
